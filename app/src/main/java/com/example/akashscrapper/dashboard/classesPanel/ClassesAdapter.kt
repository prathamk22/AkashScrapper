...
import com.example.akashscrapper.utils.sameAndEqual
import kotlinx.android.synthetic.main.class_divider_item.view.*
import kotlinx.android.synthetic.main.classes_item.view.*

class ClassesAdapter :
    ListAdapter<SemesterListModel, ClassesAdapter.ClassesViewHolder>(SemesterDC()) {

    var clickListener: ClassesClickListener? = null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = ClassesViewHolder(
        when (viewType) {
            0 -> LayoutInflater.from(parent.context).inflate(R.layout.class_divider_item, parent, false)
            else -> LayoutInflater.from(parent.context).inflate(R.layout.classes_item, parent, false)
        }
    )
